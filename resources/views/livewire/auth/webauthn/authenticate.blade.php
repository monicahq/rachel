...
new class extends Component
{
    ...
    public function mount(string $remember, bool $autofill, string $action): void
    {
        $this->remember = $remember;
        $this->autofill = $autofill;
        $this->action = $action;
        $this->publicKey = Webauthn::prepareAssertion(Auth::user());
    }
...
<div class="mt-4">
  <div wire:cloak x-data="{ run: false }">
    <flux:button icon="key" x-show="!run" @click="run = true; $dispatch('start-authenticate');" variant="primary" color="sky">
      {{ $action }}
    </flux:button>
    ...
    <div wire:show="errorMessage">
      ...
      <flux:button icon="arrow-path" wire:show="!processing" @click="$dispatch('start-authenticate');" variant="primary" color="sky">
        {{ __('Retry') }}
      </flux:button>
    </div>
  </div>
...
@script
  <script>
    ...
    const webauthnLoginCallback = async (data) => {
      await axios
        ...
        .then((response) => {
          ...
          if (response.data.result === true) {
            $wire.success = true;
            location.assign(response.data.callback);
          }
        })
        .catch((error) => {
          stop();
          $wire.errorMessage = error.message ?? error.data.errors[0];
        });
    };
  </script>
@endscript
