...
<div
  ...
  <div class="flex items-center">
    @for ($x = 1; $x <= $digits; $x++)
      <input x-ref="input{{ $x }}" type="text" inputmode="numeric" pattern="[0-9]" maxlength="1" autocomplete="off" @paste="handlePaste" @keydown="handleKeyDown({{ $x }}, $event)" @focus="$el.select()" @input="$el.value = $el.value.replace(/[^0-9]/g, '').slice(0, 1)" @class(['bg-accent-foreground text-accent-content focus:border-accent dark:focus:border-accent flex size-10 items-center justify-center border border-zinc-300 text-center text-sm font-medium transition-colors focus:relative focus:z-10 focus:border-2 focus:outline-none dark:border-zinc-700', 'rounded-l-md' => $x === 1, 'rounded-r-md' => $x === $digits, '-ml-px' => $x > 1]) />
    @endfor
  </div>

  <input {{ $attributes->except(['digits']) }} type="hidden" x-ref="code" name="{{ $name }}" minlength="{{ $digits }}" maxlength="{{ $digits }}" />
</div>
